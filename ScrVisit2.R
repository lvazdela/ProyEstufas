...
# Crea: dfpatCharFinalV2
#___________________________________

library(tidyverse)
load('Rdata/dfestufasTodasVisitas.Rda')
dfestufasV2 <- dfestufasTodasVisitas |>
  filter(visit == 2)
# guardo la base como .csv y como objeto de R
save(dfestufasV2, file = 'Rdata/dfestufasV2.Rda')
write.csv(dfestufasV2, file = 'csvs/dfestufasV2.csv')
dfestufas <- dfestufasV2 |>
...
#NOTA IMPORTANTE: CUANDO SE REVISÓ LA ESTADÍSTICA CON REBECCA, SE VIO QUE LOS VALORES SE REPETÍAN EN 
# AMBAS VISITAS. sE REVISÓ Y HABÍA UN ERROR EN ESTE CÓDIGO EN EL ScrVisit1, SE COPIABAN LOS VALORES
#ESTADÍSTICOS A DFVARCOOKING DESDE VARCOOKINGINDICE.CSV. SE RESOLVIÓ, PERO AL USAR EL MISMO .CVS QUE VISIT 1, EN VISIT2 
# AL NO MEDIR EL HUMO, f_frecCategos OMITE LA VARIABLE, ESO SE NOTÓ CUANDO CORREGÍ EL CÓDIGO INCORRECTO.
dfvarcookingInd <- dfvarcookingInd |>
  filter(variable != 'humo')
...
#df final
dfvarCookingFinalV2 <- bind_rows(dflugarCocina, dfvarCooking2Eng) |>
...
save(dfvarCookingFinalV2, file = 'Rdata/dfvarCookingFinalV2.Rda')
...
dfvarCookingPracticesV2 <- dfvarCookingFinalV2 |>
  filter(!str_starts(FinalName, '%'))
save(dfvarCookingPracticesV2, file = 'Rdata/dfvarCookingPracticesV2.Rda')
# con esta df se hace la tabla de cooking practices en el .Rmd

# COOKING METHODS WITH STATISTICS -----------------------------------------
dfvarCookingMethods <- setdiff(dfvarCookingFinalV2, dfvarCookingPracticesV2)
...
dfvarCookingMethodsV2 <- dfvarCookingMethods |>
  left_join(dflistapesCookingMeth, by = 'variable')
save(dfvarCookingMethodsV2, file = 'Rdata/dfvarCookingMethodsV2.Rda')
...
varSymptomScales <- c('grupo', 'epoctotal', 'mmrc', 'phq4total',  'psqitotal') #'psstotal',
...
dfwsympscEng <- read_csv('csvs/dfTsymptscEng.csv') |>
  filter(variable != 'psstotal')

dfTvarSymptScalFinalV2 <- dfTvarSymptomScales |>
  mutate(FinalName = dfwsympscEng$FinalName) |>
  select(FinalName, Control, Ecostove, p)
save(dfTvarSymptScalFinalV2, file =  'Rdata/dfTvarSymptScalFinalV2.Rda')
...
dfWvarSymptScalFinalV2 <- dfWvarSymptomScales |>
  left_join(dfwsympscEng) |>
  select(FinalName, Control, Ecostove, p)
save(dfWvarSymptScalFinalV2, file =  'Rdata/dfWvarSymptScalFinalV2.Rda')
...
dfpesDeChis <- data.frame(variable = varCategsymptScales,
                          p = f_roundcond(chisSymptScalCateg, 2) )
...
#Unimos las bases:
dfRespSymptFinalV2 <- bind_rows(dfcopdTitle,
                              filter(dfPorcsymptScalCat, variable == 'epoctotcateg'),
                              dfmmrcTitle,
                              filter(dfPorcsymptScalCat, variable == 'mmrccateg'))
save(dfRespSymptFinalV2, file = 'Rdata/dfRespSymptFinalV2.Rda')

dfMentalSleepFinalV2 <- bind_rows (dfphq4Title,
...
save(dfMentalSleepFinalV2, file = 'Rdata/dfMentalSleepFinalV2.Rda')
...
chisOtrosSintomas
# No se midió num_sintrespninios en visita 2
...
# 
# #Unimos las bases:
dfotrosSintomasV2 <- bind_rows(dfcefaleaTitle,
...
save(dfotrosSintomasV2, file = 'Rdata/dfotrosSintomasV2.Rda')
# 
...
dfhealthNumV2 <- dfhealth |>
  select(c(1,5, 2:4))

#df que será la de flextable
save(dfhealthNumV2, file = 'Rdata/dfhealthNumV2.Rda')
...
dfPorHealthCat$Control <- with(dfPorHealthCat,
                                   ifelse(is.na(Control), '0', Control))
